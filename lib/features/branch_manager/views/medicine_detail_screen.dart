...
class _MedicineDetailScreenState extends State<MedicineDetailScreen> {
  ...
  Future<void> _handleDeactivate() async {
    ...
    if (confirmed == true) {
      ...
      try {
        await _medicineService.deactivateMedicine(_medicine!.medicineID);
        if (mounted) {
    ...
    return PopScope(
      ...
      onPopInvokedWithResult: (bool didPop, dynamic result) {
        if (didPop && _hasChanges) {
          // Pass back true so previous screen reloads
          // Note: Flutter >= 3.12 uses onPopInvokedWithResult to handle this
        }
      },
      child: Scaffold(
          ...
          actions: [
            if (_medicine != null) ...[
              IconButton(
                ...
                onPressed: () async {
                  final result = await context.push('/manager/medicines/${_medicine!.medicineID}/edit');
                  if (result == true) {
                    _hasChanges = true;
                    _fetchDetails();
                  }
              ...
              if (_medicine!.isActive)
                IconButton(
                  icon: const Icon(Icons.block_outlined, color: Color(0xFFFF5252)),
