...
class _MedicineListScreenState extends State<MedicineListScreen> {
  ...
  Future<void> _handleDeactivate(int medicineId) async {
    ...
    if (confirmed == true) {
      ...
      try {
        await _medicineService.deactivateMedicine(medicineId);
        if (mounted) {
            ...
            SnackBar(
              content: const Row(
                children: [
                  ...
                  SizedBox(width: 8),
                  Text('Ngưng hoạt động thuốc thành công!'),
                ],
              ),
        ...
        if (mounted) {
          showDialog(
            ...
            builder: (ctx) => AlertDialog(
              ...
              content: Text(e.toString().replaceFirst('Exception: ', ''), style: const TextStyle(color: Colors.white70)),
              actions: [
                TextButton(
  ...
  Widget _buildMedicineCard(MedicineModel medicine) {
    ...
    return Card(
      ...
      child: InkWell(
        onTap: () async {
          final result = await context.push('/manager/medicines/${medicine.medicineID}');
          if (result == true) {
            _fetchMedicines();
          }
        },
              ...
              Row(
                ...
                children: [
                  ...
                  Row(
                    children: [
                      IconButton(
                        icon: const Icon(Icons.edit_outlined, color: Color(0xFF1E88E5), size: 20),
                        onPressed: () async {
                          final result = await context.push('/manager/medicines/${medicine.medicineID}/edit');
                          if (result == true) {
                            _fetchMedicines();
                          }
                        },
                      ...
                      if (medicine.isActive)
                        IconButton(
                          icon: const Icon(Icons.block_outlined, color: Color(0xFFFF5252), size: 20),
                          onPressed: () => _handleDeactivate(medicine.medicineID),
                        ),
                    ],
                  )
                ],
