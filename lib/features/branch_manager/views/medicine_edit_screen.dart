...
class MedicineEditScreen extends StatefulWidget {
  ...
  const MedicineEditScreen({super.key, required this.medicineId});
...
class _MedicineEditScreenState extends State<MedicineEditScreen> {
  ...
  Future<void> _loadAllData() async {
    ...
    try {
      // Load categories and medicine details in parallel
      final results = await Future.wait([
        _medicineService.getCategories(),
        _medicineService.getMedicineById(widget.medicineId),
      ]);

      final categoriesList = results[0] as List<CategoryModel>;
      final medicine = results[1] as MedicineDetailModel;
      ...
      setState(() {
        ...
        _priceController.text = medicine.sellingPrice.toStringAsFixed(0);
        _unitController.text = medicine.unit;
        _dosageController.text = medicine.dosageInstructions ?? '';
        _selectedCategoryId = medicine.categoryID;
